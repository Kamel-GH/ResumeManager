  ...
  return (
    ...
    >
      ...
      {selectedElementIds.map((elementId) => {
        ...
        return selectedNode ? (
          <SelectionOverlay key={`selection-${elementId}`} node={selectedNode} />
        ) : null;
      })}
    </svg>
  );
...
function TemplateImage({ node, onSelect }: { node: RenderNode; onSelect?: () => void }) {
  const { frame, props } = node;
  const src = normalizeRenderableSvgSource(
    propString(props, "src") ?? propString(props, "svg") ?? "",
  );
  ...
  const imageX =
    frame.x + frame.width / 2 + imageEditing.crop.x * frame.width * 0.5 - imageWidth / 2;
  const imageY =
    frame.y + frame.height / 2 + imageEditing.crop.y * frame.height * 0.5 - imageHeight / 2;
...
function TemplateRichText({ node, onSelect }: { node: RenderNode; onSelect?: () => void }) {
  ...
  return (
    <g
      onClick={onSelect}
      transform={buildSvgNodeTransform(frame, props, node.rotation ?? 0)}
      opacity={opacity}
    >
...
function renderImageSvgClipPath(
  ...
  switch (editing.mask.type) {
    ...
    case "ellipse":
      return (
        <ellipse cx={centerX} cy={centerY} rx={maskFrame.width / 2} ry={maskFrame.height / 2} />
      );
    case "diamond":
      return (
        <polygon
          points={`${centerX},${frame.y + maskFrame.y} ${frame.x + maskFrame.x + maskFrame.width},${centerY} ${centerX},${frame.y + maskFrame.y + maskFrame.height} ${frame.x + maskFrame.x},${centerY}`}
        />
      );
    case "star":
      return (
        <polygon
          points={buildStarSvgPoints({
            x: frame.x + maskFrame.x,
            y: frame.y + maskFrame.y,
            width: maskFrame.width,
            height: maskFrame.height,
    ...
    case "blob":
      return (
        <polygon
          points={[
            [
              frame.x + maskFrame.x + maskFrame.width * 0.44,
              frame.y + maskFrame.y + maskFrame.height * 0.03,
            ],
            [
              frame.x + maskFrame.x + maskFrame.width * 0.75,
              frame.y + maskFrame.y + maskFrame.height * 0.1,
            ],
            [
              frame.x + maskFrame.x + maskFrame.width * 0.98,
              frame.y + maskFrame.y + maskFrame.height * 0.38,
            ],
            [
              frame.x + maskFrame.x + maskFrame.width * 0.89,
              frame.y + maskFrame.y + maskFrame.height * 0.72,
            ],
            [
              frame.x + maskFrame.x + maskFrame.width * 0.61,
              frame.y + maskFrame.y + maskFrame.height * 0.96,
            ],
            [
              frame.x + maskFrame.x + maskFrame.width * 0.26,
              frame.y + maskFrame.y + maskFrame.height * 0.88,
            ],
            [
              frame.x + maskFrame.x + maskFrame.width * 0.04,
              frame.y + maskFrame.y + maskFrame.height * 0.58,
            ],
            [
              frame.x + maskFrame.x + maskFrame.width * 0.12,
              frame.y + maskFrame.y + maskFrame.height * 0.23,
            ],
          ]
...
function renderImageSvgMaskBorder(
  frame: { x: number; y: number; width: number; height: number },
  rotation: number,
  editing: ImageEditingState,
) {
    ...
    >
      ...
      {Array.from({ length: rows - 1 }, (_, index) => index + 1).map((row) => (
        <line
          ...
          x1={frame.x}
          y1={frame.y + row * cellHeight}
          x2={frame.x + frame.width}
          y2={frame.y + row * cellHeight}
          stroke="#e2e8f0"
          strokeWidth={1}
        />
      ))}
      {Array.from({ length: columns - 1 }, (_, index) => index + 1).map((column) => (
        <line
          ...
          x1={frame.x + column * cellWidth}
          y1={frame.y}
          x2={frame.x + column * cellWidth}
...
function buildSvgNodeTransform(
  frame: { x: number; y: number; width: number; height: number },
