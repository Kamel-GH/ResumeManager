...
export function CanvasWorkspaceSettingsDialog({
  open,
  onOpenChange,
  settings,
  pageMargin,
  onChange,
  onPageMarginChange,
}: CanvasWorkspaceSettingsDialogProps) {
  ...
  const workspaceModeLabel =
    workspaceModeOptions.find((option) => option.value === effectiveWorkspaceMode)?.label ??
    "Ajuster à l'espace";
  const rulerModeLabel =
    rulerModeOptions.find((option) => option.value === draftSettings.rulerMode)?.label ??
    "Règle de page";
  const rulerMajorStepDisplay = formatMeasurementValue(
    draftSettings.rulerMajorStep,
    measurementUnit,
    2,
  );
  const rulerMinorStepDisplay = formatMeasurementValue(
    draftSettings.rulerMinorStep,
    measurementUnit,
    2,
  );
  const rulerFineStepDisplay = formatMeasurementValue(
    draftSettings.rulerFineStep,
    measurementUnit,
    2,
  );
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="ef-workspace-settings-dialog">
          <div className="ef-workspace-settings-header">
            <div>
              <Dialog.Title className="ef-workspace-settings-title">
                Réglages du workspace
              </Dialog.Title>
              <Dialog.Description className="ef-workspace-settings-description">
                Contrôle le comportement du workspace et les repères visibles sans occuper tout
                l’écran.
              </Dialog.Description>
            </div>
            <div className="ef-workspace-settings-summary" aria-label="Résumé des réglages">
              ...
              <SettingsPill
                label="Snap"
                value={draftSettings.snapEnabled ? "Activé" : "Désactivé"}
              />
              <SettingsPill
                label="Grille"
                value={draftSettings.gridEnabled ? "Visible" : "Masquée"}
              />
            </div>
          </div>
          ...
          <div className="ef-workspace-settings-body" ref={bodyRef}>
            <WorkspaceSettingsSection
              title="Mesure"
              summary={measurementUnitLabel}
              description="Le moteur interne reste en px. L'unité choisie sert à afficher et saisir les valeurs."
            >
              <div className="ef-workspace-settings-grid">
                <LabeledSelect
                  ...
                  onChange={(value) =>
                    updateDraftSettings({ measurementUnit: value as MeasurementUnit })
                  }
                  options={measurementUnitOptions.map((option) => ({
                    value: option.value,
                    label: option.label,
                  }))}
                />
            ...
            <WorkspaceSettingsSection
              title="Comportement"
              summary={workspaceModeLabel}
              description="Le workspace s'adapte à l'espace disponible ou reste libre."
            >
              <div
                className="ef-workspace-settings-choice-list"
                role="radiogroup"
                aria-label="Comportement du workspace"
              >
            ...
            >
              <div className="ef-workspace-settings-grid">
                <LabeledSelect
                  ...
                  options={rulerModeOptions.map((option) => ({
                    value: option.value,
                    label: option.label,
                  }))}
                />
            ...
            <WorkspaceSettingsSection
              title="Marges de page"
              summary={pageMarginSummary}
              description="Définissent la zone utile du document dans la page active."
            >
              <div className="ef-workspace-settings-grid">
                <LabeledMeasurementField
                  label="Gauche"
                  valuePx={draftPageMargin.left}
                  measurementUnit={measurementUnit}
                  onChangePx={(value) => updateDraftPageMargin({ left: value })}
                />
                <LabeledMeasurementField
                  label="Droite"
                  valuePx={draftPageMargin.right}
                  measurementUnit={measurementUnit}
                  onChangePx={(value) => updateDraftPageMargin({ right: value })}
                />
                <LabeledMeasurementField
                  label="Haut"
                  valuePx={draftPageMargin.top}
                  measurementUnit={measurementUnit}
                  onChangePx={(value) => updateDraftPageMargin({ top: value })}
                />
                <LabeledMeasurementField
                  label="Bas"
                  valuePx={draftPageMargin.bottom}
                  measurementUnit={measurementUnit}
                  onChangePx={(value) => updateDraftPageMargin({ bottom: value })}
                />
              </div>
            </WorkspaceSettingsSection>
            ...
            >
              <div className="ef-workspace-settings-grid">
                <LabeledMeasurementField
                  label="Marge entre pages"
                  valuePx={draftSettings.pageGap}
                  measurementUnit={measurementUnit}
                  onChangePx={(value) => updateDraftSettings({ pageGap: value })}
                />
                <LabeledMeasurementField
                  label="Padding du workspace"
                  valuePx={draftSettings.pagePadding}
                  measurementUnit={measurementUnit}
                  onChangePx={(value) => updateDraftSettings({ pagePadding: value })}
                />
                <LabeledMeasurementField
                  label="Pas de grille"
                  valuePx={draftSettings.gridSize}
                  measurementUnit={measurementUnit}
                  onChangePx={(value) => updateDraftSettings({ gridSize: value })}
                />
                <div className="ef-workspace-settings-switches">
                  <WorkspaceToggle
                    label="Grille visible"
                    checked={draftSettings.gridEnabled}
                    onChange={(checked) => updateDraftSettings({ gridEnabled: checked })}
                  />
                  <WorkspaceToggle
                    label="Règles visibles"
                    checked={draftSettings.rulersVisible}
                    onChange={(checked) => updateDraftSettings({ rulersVisible: checked })}
                  />
                  <WorkspaceToggle
                    label="Marges visibles"
                    checked={draftSettings.marginsVisible}
                    onChange={(checked) => updateDraftSettings({ marginsVisible: checked })}
                  />
                  <WorkspaceToggle
                    label="Guides visibles"
                    checked={draftSettings.guidesVisible}
                    onChange={(checked) => updateDraftSettings({ guidesVisible: checked })}
                  />
                  <WorkspaceToggle
                    label="Magnétisme activé"
                    checked={draftSettings.snapEnabled}
                    onChange={(checked) => updateDraftSettings({ snapEnabled: checked })}
                  />
                  <WorkspaceToggle
                    label="Snap sur la grille"
                    checked={draftSettings.snapToGrid}
                    onChange={(checked) => updateDraftSettings({ snapToGrid: checked })}
                  />
                  <WorkspaceToggle
                    label="Snap sur les marges"
                    checked={draftSettings.snapToMargins}
                    onChange={(checked) => updateDraftSettings({ snapToMargins: checked })}
                  />
                  <WorkspaceToggle
                    label="Snap sur les bords de page"
                    checked={draftSettings.snapToPageBounds}
                    onChange={(checked) => updateDraftSettings({ snapToPageBounds: checked })}
                  />
                </div>
              </div>
              <p className="ef-workspace-settings-note">
                Espacement entre pages: {pageGapDisplay} {measurementUnitSuffix} • Padding du
                workspace: {pagePaddingDisplay} {measurementUnitSuffix}
              </p>
            </WorkspaceSettingsSection>
          </div>
...
function LabeledMeasurementField({
  ...
  const commit = () => {
    ...
    const nextPxValue = roundToTwoDecimals(
      convertMeasurementValue(nextUnitValue, measurementUnit, "px"),
    );
