...
## TARGET_REPO_TREE Governance
`TARGET_REPO_TREE.md` est une référence d’architecture cible, pas un scaffold obligatoire à créer immédiatement.

Règles :
- ne jamais créer toute l’arborescence aveuglément ;
- toujours comparer d’abord le repo réel à `TARGET_REPO_TREE.md` ;
- créer uniquement les dossiers et fichiers nécessaires au lot en cours ;
- converger progressivement vers la structure cible ;
- si le repo existant est déjà sain, adapter la cible plutôt que forcer une réécriture ;
- ne jamais créer des dossiers structurels vides sans usage immédiat ;
- ne jamais faire de déplacements/renommages massifs hors d’un lot structurel dédié.

### Ownership rules
- `src/app` = routes, layouts, pages, entrypoints ;
- `src/components` = composants UI et composants de présentation réutilisables ;
- `src/features` = orchestration métier et composition fonctionnelle par domaine ;
- `src/services` = services, intégrations et orchestration technique/métier ;
- `src/lib` = utilitaires transverses, clients partagés, helpers, constantes ;
- `src/schemas` = schémas de validation et contrats typés ;
- `src/stores` = conteneurs d’état global ou d’état de domaine ;
- `prisma` = persistence, schéma, migrations, seed.

### Anti-duplication structurelle
- ne jamais dupliquer la même responsabilité dans `components`, `features`, `services` et `lib` ;
- avant de créer un sous-dossier, vérifier quel dossier possède déjà cette responsabilité ;
- si la frontière est ambiguë, la clarifier avant implémentation.

### Migration structurelle
- toute migration structurelle importante doit être proposée comme un lot dédié ;
- séparer autant que possible les lots de structure des lots de features ;
- toujours annoncer les fichiers déplacés, imports impactés, bénéfices attendus et risques de régression.

### Frontière settings / admin
- `settings` = réglages fonctionnels, configuration métier, organisation, préférences ;
- `admin` = supervision, rôles avancés, santé système, logs, sécurité, maintenance, exploitation.
...
Les AGENTS imbriqués apportent des règles plus spécifiques pour les chemins effectivement retenus dans la structure cible :
...
Règles :
- appliquer d’abord les règles du présent fichier à tout le repo ;
- affiner ensuite avec les règles locales du dossier concerné ;
- si une variante `src/features/.../AGENTS.md` est retenue à la place, la convention doit être mise à jour partout de manière cohérente avant implémentation ;
- ne jamais maintenir deux conventions de chemins concurrentes en parallèle.
